...
import numpy as np
import pandas as pd
from make_work import Workplaces
...
class TestSchoolsCase1(unittest.TestCase):
    def setUp(self):
        ...
        t5 = time.time()
        self.w1 = Workplaces(self.people1, 100)
        t6 = time.time_ns()
    ...
    def test_init(self):
        ...
        agents_data, data_mbr = self.w1.split_sampled_data_from_agents_data(
            agents_data["ids"], w_reshaped, agents_data
        )
        self.assertEqual(
        ...
        # ensure the ages are bigger 16
        ages = self.people1.get_data_from_ids(w_reshaped, cs.AGE)
        self.assertEqual(np.where(ages <= 16, 1, 0).sum(), 0)
